...
public class NetworkingHelper {
	private static String validateUri(String targetAddress, String uri) {
		...
		for (int i = 0; i < targetAddress.length() - 1; i++) {
			if (targetAddress.charAt(i) == '/' && targetAddress.charAt(i + 1) == '/' && i > 0
					&& targetAddress.charAt(i - 1) != ':') {
				targetAddress = targetAddress.substring(0, i + 1) + (targetAddress.length() == i + 2 ? ""
						: targetAddress.substring(i + 2, targetAddress.length()));
				i--;
			}
		}
	...
	static String createRequestAddress(String targetAddress, String uri, Map<String, Object> params) {
		...
		for (String key : params.keySet()) {
			...
			try {
				requestAddress.append(URLEncoder.encode(key, "UTF-8")).append("=")
	...
	static byte[] createParamBytes(Map<String, Object> params) {
		...
		for (String key : params.keySet()) {
			...
			try {
				requestData.append(URLEncoder.encode(key, "UTF-8")).append("=")
						.append(URLEncoder.encode(value, "UTF-8")).append("&");
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
